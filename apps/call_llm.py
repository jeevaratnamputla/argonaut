import os
from openai import OpenAI
from generic_storage import get_thread_messages

# OpenAI-compatible endpoints
GEMINI_BASE_URL = "https://generativelanguage.googleapis.com/v1beta/openai/"
CLAUDE_BASE_URL = "https://api.anthropic.com/v1/"  # Anthropic's OpenAI-SDK compatible endpoint

def get_llm_response(thread_ts, max_response_tokens, temperature, logger=None):
    """
    Get a response from the available LLM (OpenAI, Claude, or Gemini)
    based on the API keys present in the environment.
    Priority: OPENAI -> CLAUDE -> GEMINI
    """
    openai_key = os.getenv("OPENAI_API_KEY")
    claude_key = os.getenv("CLAUDE_API_KEY") or os.getenv("ANTHROPIC_API_KEY")
    gemini_key = os.getenv("GEMINI_API_KEY")
    
    client = None
    model_to_use = None

    # --- 1. Determine which provider to use ---
    if openai_key:
        provider = "OpenAI"
        client = OpenAI(api_key=openai_key)
        model_to_use = os.getenv("OPENAI_MODEL", "gpt-4o-mini")

    elif claude_key:
        provider = "Claude"
        # Use Anthropic's OpenAI SDK–compatible layer
        client = OpenAI(api_key=claude_key, base_url=CLAUDE_BASE_URL)
        # Pick any current Claude model; override via env if needed
        # See Claude docs for current names (e.g., "claude-sonnet-4-5")
        model_to_use = os.getenv("CLAUDE_MODEL", "claude-sonnet-4-5")

    elif gemini_key:
        provider = "Gemini"
        client = OpenAI(api_key=gemini_key, base_url=GEMINI_BASE_URL)
        model_to_use = os.getenv("GEMINI_MODEL", "gemini-2.5-flash")

    else:
        error_msg = "No API key found for OpenAI, Claude, or Gemini. Cannot proceed."
        if logger:
            logger.error(error_msg)
        return "Error: No API key configured for any supported LLM."

    if logger:
        logger.info(f"Using provider: {provider} with model: {model_to_use}")

    # --- 2. Prepare Messages and Call the API ---
    try:
        messages_from_es = get_thread_messages(thread_ts, logger=logger)

        chat_messages = []
        for msg in messages_from_es:
            chat_messages.append({
                "role": msg.get("role", ""),
                "content": msg.get("content", "")
            })

        completion = client.chat.completions.create(
            messages=chat_messages,
            model=model_to_use,
            max_tokens=max_response_tokens,
            temperature=temperature,
            top_p=float(os.getenv("top_p", 0.5))
        )

        response_content = completion.choices[0].message.content

        if logger:
            logger.debug(f"{provider} completion response: %s", completion.choices[0])
            logger.info(f"{provider} responded successfully.")

        return response_content

    except Exception as e:
        if logger:
            logger.error(f"Error in get_llm_response with {provider}: {e}")
        return "Error processing your request."
